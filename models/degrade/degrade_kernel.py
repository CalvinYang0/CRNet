...
def get_raw2rgb2(img, features, demosaic='net', lineRGB=False):
    # img = np.transpose(img, (1, 2, 0))
    # img = torch.from_numpy(np.array(img))
    image=[]
    ...
    device = img.device
    for i in range(img.shape[0]):
        im=image[i].unsqueeze(0)
        deg_img = process(im, features['red_gain'][i].unsqueeze(0).to(device), features['blue_gain'][i].unsqueeze(0).to(device),
                      features['cam2rgb'][i].unsqueeze(0).to(device), demosaic, lineRGB)
        deg_img = deg_img.squeeze(0)
        deg_images.append(deg_img)
    result=torch.stack(deg_images)
    # deg_img = deg_img.squeeze(0)
    # deg_img = torch.clamp(deg_img * 255.0, 0.0, 255.0).numpy()
    # deg_img = deg_img.astype(np.uint8)
    return result
