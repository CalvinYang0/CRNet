...
class BracketIREDataset(BaseDataset):
    def __init__(self, opt, split='train', dataset_name='BracketIRE'):
        ...
        if split == 'train':
            ...
            self.names, self.meta_dirs, self.raw_dirs, self.gt_dirs = self._get_image_dir(self.root, split,
                                                                                          name='Train')
            self.len_data = 50000 * self.batch_size
        elif split == 'test':
    ...
    def _getitem_train(self, idx):
        ...
        meta_data = np.load(self.meta_dirs[idx], allow_pickle=True)
